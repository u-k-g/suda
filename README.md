...
Vimium is a browser extension that provides keyboard-based navigation and control of the web. This
fork adds a first-class Helix keybinding mode while retaining Vimium's classic bindings.
...
Vimium's Options page can be reached via a link on the help dialog (type `Space h` in Helix mode or
`?` in Vimium classic mode), or via the button next to Vimium on the extension pages of Chrome and
Edge (`chrome://extensions`), or Firefox (`about:addons`).

## Helix Mode

Helix mode is selected by default on fresh installs. The Options page retains a **Vimium classic**
profile for switching back at any time, and custom mappings are layered over whichever profile is
selected.
    ...
    Ctrl-f/b      scroll a full page down/up
    Ctrl-o/i      go backward/forward in browser history
    gn/gp         go to the next/previous tab
    v             enter Select mode
    x             select the current line
    Space f/F     pick a link to open in this/a new tab
    Space b       open the tab picker
    Space /       open a URL, bookmark, history entry, or search
...
In Vimium classic mode, you can see this list of key bindings at any time by typing `?`.
...
- The **New tab URL** option controls tabs opened with the browser's `cmd-t`/`ctrl-t` shortcut or
  new-tab button, as well as Vimium's `createTab` command.
- Select **Vimium blank new tab page** to make commands available on new tabs. The options page can
  also make the Vomnibar open automatically there.
- Select **Browser's default new tab page** to leave the browser page untouched. Browser security
  prevents Vimium commands from running on that protected page.
- Select **Custom URL** to open a site of your choice instead.
